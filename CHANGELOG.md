...
- Cleaner uninstall process with proper restoration of changes.# Changelog
## I am not responsible for any unofficial or tampered versions of my module distributed outside this repository.
...
- Added "Enable all" / "Disable all" buttons, only touching categories not already in the target state.
### Deep Doze
- Added a Minimum tier below Moderate: gentler timeouts, `frequent` standby bucket, no `WAKE_LOCK`/`RUN_ANY_IN_BACKGROUND` restriction, no sensor disable, no wakelock killer. Still forces idle promptly on lock.
- Restrictions now follow lock state, not screen state, with a screen-wakefulness fallback for devices with no lockscreen.
- Screen/lock detection is now event-driven with polling fallback.
- Default dialer, SMS app, keyboard, and home launcher detected live instead of a static list: whitelist trimmed accordingly. Kept what's still static.
- Screen monitor start/stop uses an atomic lock instead of a bare PID check.
- Fixed the wakelock killer's 5-minute countdown counting loop iterations instead of elapsed time.
- All tiers now force idle immediately on lock instead of only after the 5-minute wakelock-killer countdown.
- Fixed motion sensors staying disabled after a Maximum monitor is stopped and replaced mid-session.
- Fixed `am get-standby-bucket` comparison using numeric codes instead of names.
- Wakelock killer now skips alarm/notification/push-tagged wakelocks.
### Screen Off Optimization
...
### Import / Export
- Added a Bug Report option to the WebUI menu (Language, Import/Export, Bug Report, About): generates a device/config/log dump to `Frosty/Logs`, no personal info (accounts, IMEI, phone number, Wi-Fi, location) included. `bug_report.yml` template updated to point here instead of a manual Termux command.
- Fixed the bug report generator: a grep error (invalid `\s` regex on this device's grep) was leaking into the file since only `dumpsys`'s stderr was redirected, not grep's own. Battery status/health now decoded to readable text instead of raw codes. Empty sections (no frozen GMS services, no logs yet) now say so explicitly instead of leaving blank space. Log file discovery switched to `find` for reliability. Menu icon switched from hardcoded colors to `currentColor`, matching the other menu icons' style.
- Root version dropped from the bug report (always unresolved on some KSU builds); kept root type only. Fixed installed-module versions showing doubled `v` when the module's own `version=` already includes one. Added ROM/firmware detection (MIUI/HyperOS, LineageOS, ColorOS/OxygenOS, One UI, Pixel Experience, falling back to build display ID + signing tags).
- Fixed free-text imported values not validated against their allowed set (potential command injection via a crafted backup).
- Added a legacy GMS Doze migration on import, and on in-place update.
- Import now writes config files atomically instead of leaving them truncated if killed mid-write.
### Kill Tracking
- `limit_ad_tracking` now backs up and restores your prior value instead of deleting it on revert.
- Removed the GMS background-data blacklist call, which conflicted with the Doze whitelist's push-delivery guarantee.
### Install
...
- `ram_clean_whitelist.txt` now preserved on update.
### System Props
- Removed `ro.config.small_battery`/`ro.telephony.call_ring.multiple` (not debug/logging properties).
- Blur disable reworked against a real reference module: fixed a polarity bug (`ro.sf.blurs_are_expensive` was backwards), added the full OEM property set and the real `cmd window`/`cmd wm disable-blur` command.
### Misc
- Added hot install request on KernelSU/APatch.
### Uninstall
- Fixed RAM Optimizer's kernel/ZRAM backup never being restored.
- Fixed four Kill Logs settings not reverted.
- Fixed Screen Off Optimization state and App Doze XML overlays being read from a path already deleted by the time the runner executes.
