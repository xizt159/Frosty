...
_load_patches() {
  ...
  [ ! -f "$PATCHES_BACKUP" ] && {
    ...
    printf "### Doze Patches - $(date '+%Y-%m-%d %H:%M:%S')\n" > "$PATCHES_BACKUP"
  }
  ...
  if [ "$NEW_PKGS" != "$OLD_PKGS" ]; then
    ...
    {
      echo "### Doze Patches - $(date '+%Y-%m-%d %H:%M:%S')"
      printf "%s\n" $NEW_PKGS 
    } > "$PATCHES_BACKUP"
  else
...
log_status() {
  ...
  [ -f "$OVERLAYS_FILE" ] && overlay_count=$(grep -c '^[[:space:]]*[^#[:space:]]' "$OVERLAYS_FILE" 2>/dev/null)
...
patch_xml() {
  if _is_patched; then
    local existing=0
    [ -f "$OVERLAYS_FILE" ] && existing=$(grep -c '^[[:space:]]*[^#[:space:]]' "$OVERLAYS_FILE" 2>/dev/null)
    ...
    for _dir in "$_p/etc" "$_p/oplus" "$_p/oppo"; do
      ...
      for xml in $(find "$_dir" -type f -name "*.xml" -depth -maxdepth 2 2>/dev/null); do
        ...
        if cp -af "$real" "$dest" 2>/dev/null; then
          sed -i "$_GMS_SED" "$dest"
          [ -s "$OVERLAYS_FILE" ] || {
            echo "### Frosty — GMS Overlays (autogenerated)"
            echo "### Note: should not be touched!"
          } > "$OVERLAYS_FILE"
...
apply() {
  ...
  log_doze "[INFO] Persistent <wl> removed from deviceidle.xml: $_patched (fully optimized)"
