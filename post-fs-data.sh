...
if [ "$ENABLE_GMS_DOZE" = "1" ]; then
  ...
  # Early bind mount of patched sysconfig XMLs — must happen before system_server
  ...
  while IFS= read -r _xml; do
    ...
    if grep -qE 'allow-in-power-save.*com\.google\.android\.gms|allow-in-data-usage-save.*com\.google\.android\.gms' "$_xml" 2>/dev/null; then
